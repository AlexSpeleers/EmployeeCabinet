...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccountService
    {
        public async Task<GeneralResponse> CreateAsync(Register user)
        {
            ...
            if (checkUser is not null) return new GeneralResponse(true, "User registered already.");

            var applicationUser = await SaveUser(user);
            ...
            if (checkAdminRole is null) 
            {
                ...
                return new GeneralResponse(true, "Account crated.");
            }
            
            var checkUserRole = await appDbContext.SystemRoles.FirstOrDefaultAsync(_ => _.Name!.Equals(Helpers.Constants.User));
            SystemRole responce = new();
            if (checkUserRole is null)
            {
                responce = await AddToDatabase(new SystemRole() { Name = Constants.Admin });
                await AddToDatabase(new UserRole() { RoleId = responce.Id, UserId = applicationUser.Id });
            }
            else
        ...
        public async Task<LoginResponse> SignInAsync(Login user)
        {
            ...
            if (applicationUser is null) return new LoginResponse(false, "User no found.");
        ...
        private async Task<ApplicationUser> SaveUser(Register user)
        {
